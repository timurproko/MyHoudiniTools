...
def splitver(string,frmats):
    ...
    for fmt in frmats:
        pattern = rf"{re.escape(fmt)}(?=\d+)"
        if re.search(pattern, string):
            sec = re.split(pattern, string)
...
def buildpath(padding,path,matchlist,frmat,sec):
    ...
    return '%s/%s_%s%s'%(path[0],sec[0],v,sec[2])
...
def incsave():
    ...
    sec,frmat = splitver(path[1],['_v','_V','_'])
    
    if sec:
        matchlist = [sec[1]]
        padding = len(sec[1])
        matchlist = foldersearch(path,matchlist,['_v','_V','_'],sec)[0]
